...
import { spawn, spawnSync, exec as execCb } from 'node:child_process';
...
/**
 * Derive a simple narrative arc label from an emotion arc array.
 */
function deriveArc(emotionArc = []) {
  if (!Array.isArray(emotionArc) || emotionArc.length === 0) return 'quiet';
  const intensities = emotionArc.map(e => Number(e?.intensity || 0));
  const avg = intensities.reduce((s, v) => s + v, 0) / intensities.length;
  const min = Math.min(...intensities);
  const max = Math.max(...intensities);
  const delta = max - min;
  if (avg < 0.35 && delta < 0.15) return 'quiet';
  if (delta > 0.25 && max > 0.65) return 'build';
  if (max > 0.75 && avg >= 0.5) return 'tension';
  return 'release';
}

/**
 * Collect dominant mood keywords from analysis results.
 */
function deriveDominantMood(analysisResults = []) {
  const counts = {};
  for (const a of analysisResults) {
    const mood = (a?.mood || a?.emotion || '').toString().toLowerCase();
    if (mood) counts[mood] = (counts[mood] || 0) + 1;
    if (Array.isArray(a?.tags)) {
      for (const tag of a.tags) {
        const t = String(tag || '').toLowerCase();
        if (t) counts[t] = (counts[t] || 0) + 0.25;
      }
    }
  }
...
/**
 * Map mood signals to a music mood bucket.
 */
function mapMusicMoodFromSignals(moods = []) {
  const text = moods.join(' ').toLowerCase();
  if (/(quiet|empty|fog|mist|haze|still|lonely)/.test(text)) return 'ambient';
  if (/(architecture|geometry|lines|grid|structure|minimal)/.test(text)) return 'minimal';
  if (/(contrast|tension|dramatic|storm|intense|dark)/.test(text)) return 'cinematic';
  if (/(cloud|release|sky|open|uplift|light)/.test(text)) return 'piano';
  return 'ambient';
}

/**
 * Build canonical StoryPlan object that becomes the single source of truth.
 */
function buildStoryPlan(analysisResults, sequencePlan, photos) {
  const uploadOrderIds = photos.map((p, i) => p.id || p.filename || p.originalName || `image_${i}`);
  const orderedIdsRaw = Array.isArray(sequencePlan?.ordered_ids) ? sequencePlan.ordered_ids : [];
  let aiOrderIds = [];

  if (orderedIdsRaw.length === uploadOrderIds.length) {
    aiOrderIds = orderedIdsRaw.map(id => {
      const asNumber = Number(id);
      if (Number.isInteger(asNumber) && asNumber >= 0 && asNumber < uploadOrderIds.length) {
        return uploadOrderIds[asNumber];
      }
      const matchIdx = analysisResults.findIndex(a => String(a.id) === String(id));
      if (matchIdx >= 0 && matchIdx < uploadOrderIds.length) {
        return uploadOrderIds[matchIdx];
      }
      const filenameIdx = photos.findIndex(p => (p.filename || p.originalName) === id);
      if (filenameIdx >= 0) {
        return uploadOrderIds[filenameIdx];
      }
      return null;
    }).filter(Boolean);
  }

  if (aiOrderIds.length !== uploadOrderIds.length) {
    console.warn('[STORY_PLAN] Invalid or partial ordered_ids, falling back to upload order');
    aiOrderIds = uploadOrderIds;
  }
  const validateOk = aiOrderIds.length === uploadOrderIds.length && new Set(aiOrderIds).size === uploadOrderIds.length;
  console.log(`[STORY_PLAN] validateOrder=${validateOk} {aiLen:${aiOrderIds.length}, uploadLen:${uploadOrderIds.length}}`);
  if (!validateOk) {
    console.warn("[STORY_PLAN] Falling back to upload order due to validation failure");
    aiOrderIds = uploadOrderIds;
  }

  const dominantMood = deriveDominantMood(analysisResults);
  const arc = deriveArc(sequencePlan?.emotion_arc);
  const musicMood = mapMusicMoodFromSignals(dominantMood);
  return {
    orderedImageIds: aiOrderIds,
    arc,
    dominantMood,
    musicMood,
    uploadOrderIds,
...
const MOOD_KEYWORDS = {
  ambient: ['ambient', 'atmo', 'atmos', 'drone', 'space', 'minimal', 'calm', 'quiet'],
  piano: ['piano', 'keys', 'nocturne', 'soft'],
  cinematic: ['cinematic', 'score', 'epic', 'orches', 'orchestra', 'trailer'],
  minimal: ['minimal', 'subtle', 'sparse', 'mono', 'light'],
};
...
function resolveMusicPath(storyPlan = null, options = {}) {
  try {
    const planMood = normalizeStr(storyPlan?.musicMood);
    const planMoodLog = planMood ? ` (storyPlan.musicMood=${planMood})` : '';
    const jobId = options.jobId || options.renderId || Date.now().toString();
    console.log(`[MUSIC] desiredMood=${planMood || 'none'} jobId=${jobId}`);

    const assetsDir = findAssetsDir();
    if (!assetsDir) {
      console.warn('[MUSIC] assets directory not found');
      return null;
    }
    ...
    const fallbackPick = () => {
      const files = getAvailableMusicFiles();
      if (files?.length) {
        if (planMood) {
          const moodMatch = files.find(f => path.basename(f).toLowerCase().includes(planMood));
          if (moodMatch) {
            console.log(`[MUSIC] Fallback music file (mood match${planMoodLog}): ${moodMatch}`);
            return moodMatch;
          }
        }
        const candidate = files[0];
        console.log(`[MUSIC] Fallback music file${planMoodLog}: ${candidate}`);
        return candidate;
      }
      return null;
    };
    ...
    if (existsSync(manifestPath)) {
      try {
        ...
        console.warn('[MUSIC] Failed to parse manifest.json:', e.message);
      }
    }

    if (!manifest || !(Array.isArray(manifest.tracks) || Array.isArray(manifest))) {
      const picked = fallbackPick();
      console.log(`[MUSIC] manifest missing. desiredMood=${planMood} chosen=${picked}`);
      return picked;
    }

    const tracks = Array.isArray(manifest.tracks) ? manifest.tracks : manifest;

    if (!planMood) {
      const picked = fallbackPick();
      console.log(`[MUSIC] no desiredMood. chosen=${picked}`);
      return picked;
    }

    const keywords = MOOD_KEYWORDS[planMood] || [planMood];

    const candidates = tracks.filter(t => {
      const mood = normalizeStr(t.mood);
      const tags = Array.isArray(t.tags) ? t.tags.map(normalizeStr).join(' ') : normalizeStr(t.tags);
      const file = normalizeStr(t.file);
      const title = normalizeStr(t.title);
      if (mood && mood === planMood) return true;
      const haystack = `${mood} ${tags} ${file} ${title}`;
      return keywords.some(k => haystack.includes(k));
    });

    const chosen = pickDeterministic(candidates, `${jobId}:${planMood}`) || null;

    if (chosen?.file) {
      const finalPath = path.join(assetsDir, chosen.file);
      console.log(`[MUSIC] desiredMood=${planMood} candidates=${candidates.length} chosen=${chosen.file}`);
      return finalPath;
    }

    if (chosen) {
      const fileGuess = chosen.path || chosen.filename || chosen.name;
      if (fileGuess) {
        const finalPath = path.join(assetsDir, fileGuess);
        console.log(`[MUSIC] desiredMood=${planMood} candidates=${candidates.length} chosen=${fileGuess}`);
        return finalPath;
      }
    }

    const picked = fallbackPick();
    console.log(`[MUSIC] no mood match. desiredMood=${planMood} fallback chosen=${picked}`);
    return picked;
  } catch (err) {
    console.warn('[MUSIC] resolveMusicPath failed:', err.message);
  }
  console.warn('[MUSIC] No music track available; proceeding without audio');
  return null;
}
 ...
 */
async function renderFromPlan(plan, photoPathsMap, outputPath, memoryId, photos, fps = 24, outputWidth = 1920, outputHeight = 1080, progressReporter = null, movementMode = 'OFF') {
  // #region agent log
  ...
  const uploadedCount = Array.isArray(photos) ? photos.length : 0;
  console.log(`[RENDER] Uploaded images count: ${uploadedCount}`);
  if (uploadedCount <= 0) {
    throw new Error('No photos provided for render');
  }
  const defaultSegmentDuration = 2.5;
  ...
  // Enforce full coverage: one segment per uploaded image
  const needsRebuild =
    plan.order.length !== uploadedCount ||
    plan.selected.length !== uploadedCount ||
    !Array.isArray(plan.ordered_ids) ||
    plan.ordered_ids.length !== uploadedCount;
  if (needsRebuild) {
    console.warn('[RENDER] Plan counts mismatched uploaded count. Rebuilding plan to cover ALL images.');
    plan.selected = Array.from({ length: uploadedCount }, (_, i) => i);
    plan.order = Array.from({ length: uploadedCount }, (_, i) => i);
    plan.ordered_ids = Array.from({ length: uploadedCount }, (_, i) => String(i));
    plan.shots = Array.from({ length: uploadedCount }, (_, i) => ({
      id: plan.ordered_ids[i],
      purpose: 'build',
      target_emotion: { primary: 'calm', secondary: '' },
      motion_hint: 'hold'
    }));
    plan.durations = Array.from({ length: uploadedCount }, () => defaultSegmentDuration);
    plan.transitions = Array.from({ length: Math.max(0, uploadedCount - 1) }, () => 'crossfade');
  }
  
  const expectedCount = plan.order.length;
  if (plan.order.length !== plan.durations.length) {
    console.warn(`[RENDER] durations length mismatch; fixing to order length (${expectedCount}) with default ${defaultSegmentDuration}s`);
    plan.durations = Array.from({ length: expectedCount }, () => defaultSegmentDuration);
  }
  
  if (plan.transitions.length !== Math.max(0, expectedCount - 1)) {
    console.warn(`[RENDER] transitions length mismatch; fixing to ${Math.max(0, expectedCount - 1)} crossfades`);
    plan.transitions = Array.from({ length: Math.max(0, expectedCount - 1) }, () => 'crossfade');
  }
  ...
  console.log(`[RENDER] Photo mapping (CRITICAL: using AI order, NOT upload order):`);
    ...
    if (plan.storyPlan && Array.isArray(plan.storyPlan.uploadOrderIds)) {
      const renderIds = orderedPhotoFilenames;
      const uploadIds = plan.storyPlan.uploadOrderIds;
      const aiIds = plan.storyPlan.renderImageIds || plan.storyPlan.orderedImageIds || uploadIds;
      console.log(`[ORDER] uploadIds = ${uploadIds.join(',')}`);
      console.log(`[ORDER] aiIds     = ${aiIds.join(',')}`);
      console.log(`[ORDER] renderIds = ${renderIds.join(',')}`);
      if (renderIds.join(',') !== aiIds.join(',')) {
        throw new Error("Render is NOT using AI order. Wiring bug.");
      }
      console.log(`[RENDER]   STORY PLAN upload order ids: [${uploadIds.join(', ')}]`);
      if (renderIds.join(',') === uploadIds.join(',')) {
        console.warn('⚠️ RENDER ORDER === UPLOAD ORDER — verify OpenAI storytelling order');
      }
    }
  console.log(`[RENDER]   Expected segment count: ${expectedCount}`);
  console.log(`[RENDER] ========================================`);
  ...
  const coverOnlyFilter = (w, h, sigma = null) => {
    let f = `scale=${w}:${h}:force_original_aspect_ratio=increase:flags=lanczos+accurate_rnd,crop=${w}:${h},setsar=1,scale=${w}:${h}`;
    if (sigma && sigma > 0) f += `,gblur=sigma=${sigma}`;
    f += `,format=yuv420p`;
    return f;
  };
  ...
  for (let i = 0; i < orderedFiles.length; i++) {
    ...
    // Gallery pacing: clamp to 3.0–4.5s per image
    duration = Math.max(3.0, Math.min(4.5, duration || 3.5));
    ...
    if (movementMode === 'OFF') {
      motionPlanData = {
        ...
        movementMode: 'OFF',
        noMotion: true,
        antiFlicker: motionPlanData?.antiFlicker || false
      };
    }
    ...
    let templateConfig;
    if (movementMode === 'OFF') {
      const filter = coverOnlyFilter(outputWidth, outputHeight, null);
      templateConfig = {
        duration,
        inputs: ['-loop', '1', '-i', photoPath],
        filter,
        ...
        metadata: {
          ...
          beatPosition: totalSegments > 1 ? i / (totalSegments - 1) : 0.5,
          filename: photoFilename
        }
      };
    } else {
      templateConfig = getTemplate('ken_burns', photoPath, duration, fps, i, outputWidth, outputHeight, photoMeta, totalSegments, motionPlanData);
    }
    const usesZoompan = templateConfig.filter.includes('zoompan');
    console.log(`[SEGMENT][${i}] id=${photoFilename} motionMode=${motionPlanData?.motionMode || 'n/a'} antiFlicker=${!!motionPlanData?.antiFlicker} usesZoompan=${usesZoompan}`);
    if (movementMode === 'OFF' && usesZoompan) {
      throw new Error(`[NO-MOTION] Segment ${i} incorrectly includes zoompan`);
    }
  ...
  console.log(`[SEGMENTS] All ${segments.length} segments created successfully (expected ${expectedCount})`);
  if (segments.length !== expectedCount) {
    throw new Error(`Render mismatch: expected ${expectedCount} segments, rendered ${segments.length}`);
  }
  
  // Premium gallery transitions: uniform cross-dissolve ~350ms (at 24fps ~8-14 frames)
  const transitions = [];
...
function buildXfadeFilter(segments, segmentDurations, transitions, fps = 24) {
  ...
  // Build running composed duration so offsets reflect prior overlaps
  // Each xfade overlaps by trans.duration; we track currentDuration as composed length so far.
  let currentDuration = segmentDurations[0]; // after first segment, before any transition
  console.log(`[XFADE] Initial composed duration: ${currentDuration.toFixed(3)}s`);
  ...
  for (let i = 0; i < transitions.length; i++) {
    ...
    const xdur = Math.max(0, Number(trans.duration || 0));
    if (xdur <= 0) {
      throw new Error(`Invalid xfade duration at transition ${i}: ${xdur}`);
    }
    ...
    // Calculate offset on the composed timeline (iterative):
    // offset_i = currentDuration - xdur
    const offset = Math.max(0, currentDuration - xdur);
    if (offset >= currentDuration) {
      throw new Error(`Invalid xfade offset: offset=${offset} >= currentDuration=${currentDuration} at transition ${i}`);
    }
    
    console.log(`[XFADE] Transition ${i + 1}: ${trans.type} (${trans.duration}s)`);
    console.log(`[XFADE]   From seg ${trans.fromIndex} (timeline ${currentDuration.toFixed(2)}s) to seg ${trans.toIndex}`);
    console.log(`[XFADE]   Offset: ${offset.toFixed(2)}s = currentDuration - xdur`);
    ...
    currentLabel = outputLabel;
    // Update composed duration: add next segment, subtract overlap once
    currentDuration = currentDuration + segmentDurations[trans.toIndex] - xdur;
  }
 ...
 * @param {string|null} musicPath - Optional absolute path to music track
 */
async function renderWithXfade(ffmpegPath, segments, segmentDurations, transitions, outputPath, fps = 24, outputWidth = 1920, outputHeight = 1080, musicPath = null, movementMode = 'OFF') {
  ...
  if (musicPath) {
    if (existsSync(musicPath)) {
      ...
      audioInputIndex = segments.length; // music is last input
      console.log(`[MUSIC] Attaching music track: ${musicPath}`);
    } else {
      console.warn(`[MUSIC] Music path not found on disk: ${musicPath}`);
    }
  }
  ...
  if (segments.length === 1 && audioInputIndex === null) {
    // Single segment, no transitions and no music -> direct copy
    console.log(`[VIDEO] Single segment, copying directly...`);
    fs.copyFileSync(segments[0], outputPath);
    return;
  }
  
  // Build filter chain (xfade for multi-segment, simple map for single-segment)
  let filterComplex;
  let finalLabel;
  if (segments.length === 1) {
    ...
    const built = buildXfadeFilter(segments, segmentDurations, transitions, fps);
    filterComplex = built.filterComplex;
    finalLabel = built.finalLabel;
  }
  console.log("[FFMPEG] filter_complex=\n", filterComplex);
  if (movementMode === 'OFF') {
    const forbidden = ['zoompan', 'rotate', 'perspective', 'translate'];
    const hits = forbidden.filter(k => filterComplex.includes(k));
    if (hits.length) {
      throw new Error(`[NO-MOTION] Forbidden filters found: ${hits.join(', ')}`);
    }
  }
  ...
  // Optional audio fade-in/out if music is present
  if (audioInputIndex !== null) {
    const audioFadeIn = 0.7;
    const audioFadeOut = 1.5;
    const audioFadeOutStart = Math.max(0, timelineSeconds - audioFadeOut);
    finalFilter = `${finalFilter};[${audioInputIndex}:a]afade=t=in:st=0:d=${audioFadeIn.toFixed(3)},afade=t=out:st=${audioFadeOutStart.toFixed(3)}:d=${audioFadeOut.toFixed(3)}[aout]`;
  }
  ...
  try {
    ...
    // Verify audio stream if music was requested
    if (audioInputIndex !== null) {
      try {
        const probeCmd = `ffprobe -v error -show_streams -select_streams a -of compact=p=0:nk=1 "${outputPath}"`;
        const { stdout } = await exec(probeCmd);
        if (!stdout || !stdout.includes('audio')) {
          throw new Error('Audio stream missing after mux');
        }
      } catch (probeErr) {
        console.error('[MUSIC] Audio verification failed:', probeErr.message);
        throw new Error(`Audio track missing in final output (${probeErr.message})`);
      }
    }
  } catch (error) {
...
function renderSegment(ffmpegPath, templateConfig, outputPath, segmentIndex = 0, photoMetadata = null) {
  ...
  if (segmentIndex === 0) {
    ...
    console.log(`[SEGMENT][${segmentIndex}]   Has zoompan (motion): ${hasZoompan ? '✓' : '✗'}`);
    ...
    const motionModeMeta = templateConfig.metadata?.motionMode || templateConfig.metadata?.preset || 'HOLD';
    const motionModeUpper = String(motionModeMeta || 'HOLD').toUpperCase();
    const movementModeUpper = 'ON'; // movementMode not passed here; default to ON to avoid false positives
    const wantsMotion = movementModeUpper !== 'OFF' && motionModeUpper !== 'HOLD' && motionModeUpper !== 'NONE';
    console.log("[PATCH-CHECK] renderSegment validator v2 active @2032");
    console.log(`[SEGMENT][${segmentIndex}] wantsMotion=${wantsMotion} movementMode=${movementModeUpper} motionMode=${motionModeUpper} hasZoompan=${hasZoompan}`);
    if (wantsMotion && !hasZoompan) {
      throw new Error(`[SEGMENT][${segmentIndex}] Motion requested but zoompan missing. movementMode=${movementModeUpper} motionMode=${motionModeUpper}. Filter: ${templateConfig.filter}`);
    }
    if (!wantsMotion && hasZoompan) {
      throw new Error(`[SEGMENT][${segmentIndex}] Motion OFF/HOLD but zoompan present. movementMode=${movementModeUpper} motionMode=${motionModeUpper}. Filter: ${templateConfig.filter}`);
    }
  }
...
app.post('/api/create-memory', async (req, res) => {
  ...
  try {
    ...
    const { promptText, photos, sessionId, fps, outputRatio, debugReverseOrder, movementMode } = req.body;
    const debugReverse = debugReverseOrder === true || debugReverseOrder === 'true';
    const globalMovementMode = String(movementMode || 'OFF').toUpperCase() === 'ON' ? 'ON' : 'OFF';
    console.log(`[DEBUG] debugReverseOrder flag: ${debugReverse}`);
    console.log(`[DEBUG] movementMode: ${globalMovementMode}`);
    ...
    console.log(`[CREATE-MEMORY] movementMode=${globalMovementMode}`);
    ...
    try {
      motionPlan = generateMotionPlan(analysisResults, sequencePlan, storyLock);
    ...
    // Canonical story plan (single source of truth for order + mood)
    const baseStoryPlan = buildStoryPlan(analysisResults, sequencePlan, photos);
    const uploadOrderIds = baseStoryPlan.uploadOrderIds;
    const baseAiOrderIds = baseStoryPlan.orderedImageIds;
    const debugIds = [...uploadOrderIds].reverse();
    const chosenIds = debugReverse ? debugIds : baseAiOrderIds;

    console.log(`[ORDER] UPLOAD ORDER: [${uploadOrderIds.join(', ')}]`);
    console.log(`[ORDER] AI ORDER (filenames): [${baseAiOrderIds.join(', ')}]`);
    console.log(`[ORDER] DEBUG FORCED ORDER: [${debugIds.join(', ')}]`);
    console.log(`[ORDER] CHOSEN ORDER (render ids): [${chosenIds.join(', ')}]`);

    const aiOrderIndices = chosenIds.map(id => {
      const idx = uploadOrderIds.indexOf(id);
      if (idx === -1) throw new Error(`Story plan id ${id} not found in uploads`);
      return idx;
    });
    if (new Set(chosenIds).size !== chosenIds.length) {
      throw new Error(`[VALIDATION] Story plan contains duplicate ids: [${chosenIds.join(', ')}]`);
    }
    if (chosenIds.length !== uploadOrderIds.length) {
      throw new Error(`[VALIDATION] Story plan length mismatch: ordered=${chosenIds.length} uploads=${uploadOrderIds.length}`);
    }
    if (!debugReverse && baseAiOrderIds.join(',') === uploadOrderIds.join(',')) {
      console.warn("⚠️ Story order equals upload order — verify OpenAI result");
    }
    ...
    const defaultSegmentDuration = 3.8;
    let plan = {
      selected: Array.from({ length: photos.length }, (_, i) => i),
      order: aiOrderIndices,
      ordered_ids: chosenIds, // filenames in story order (aligns to uploaded array)
      durations: Array.from({ length: aiOrderIndices.length }, () => defaultSegmentDuration),
      transitions: Array.from({ length: aiOrderIndices.length - 1 }, () => 'crossfade'),
      chapterCuts: {
        arrivalEnd: Math.floor(aiOrderIndices.length * 0.2),
        recognitionEnd: Math.floor(aiOrderIndices.length * 0.4),
        intimacyEnd: Math.floor(aiOrderIndices.length * 0.6),
        pauseEnd: Math.floor(aiOrderIndices.length * 0.8)
      },
      memoryNote: sequencePlan.theme || promptText || '',
      usedPlanner: 'ai',
      ...
      storyLock: storyLock,
      storyPlan: { ...baseStoryPlan, renderImageIds: chosenIds, debugReverseOrder: debugReverse }
    };
    ...
    console.log(`[CREATE-MEMORY]   Ordered IDs (indices): [${plan.order.join(',')}]`);
    ...
    console.log(`[CREATE-MEMORY]   STORY PLAN orderedImageIds: [${plan.storyPlan?.orderedImageIds?.join(', ') || ''}]`);
    console.log(`[CREATE-MEMORY]   STORY PLAN musicMood: ${plan.storyPlan?.musicMood || 'unknown'} arc=${plan.storyPlan?.arc || 'unknown'}`);
    console.log('[CREATE-MEMORY] ========================================');
    ...
    if (plan.storyPlan) {
      console.log(`[PLANNER] storyPlan.orderedImageIds=[${plan.storyPlan.orderedImageIds.join(', ')}]`);
      console.log(`[PLANNER] storyPlan.renderImageIds=[${plan.storyPlan.renderImageIds?.join(', ') || ''}] debugReverse=${plan.storyPlan.debugReverseOrder}`);
      console.log(`[PLANNER] storyPlan.musicMood=${plan.storyPlan.musicMood} arc=${plan.storyPlan.arc}`);
    }
    ...
    await renderFromPlan(plan, photoPathsMap, outputPath, sessionId, photos, targetFps, outputWidth, outputHeight, progressReporter, globalMovementMode);
    ...
    console.log("[S3] about to upload", {
      outputPath,
      videoFilename,
      bucket: S3_BUCKET,
      region: AWS_REGION
...
app.post('/api/generate-video', async (req, res) => {
  try {
    ...
    await renderFromPlan(plan, photoPathsMap, outputPath, sessionId, photos, 24, 1920, 1080, null, 'OFF');
...
// CloudFront signed URL endpoint (must be registered before 404)
app.get('/api/media/signed-url', (req, res) => {
  try {
    ...
    const { signedUrl, expiresInSeconds, url } = signVideoPath(resourcePath);
    ...
    console.error('[CLOUDFRONT] Failed to sign URL:', err);
    const status =
      err?.message?.includes('path') || err?.message?.includes('CLOUDFRONT')
        ? 400
        : 500;
    return res.status(status).json({
      error: 'Failed to sign CloudFront URL',
